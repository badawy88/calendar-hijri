...
(function($) { // Hide scope, no $ conflict
 ...
 $(selector).calendarsPicker({minDate: 0, maxDate: '+1m +1w'}) */
	$.JQPlugin.createPlugin({
		...
		defaultRenderer: {picker: '<div class="calendars">' +
			'<div class="calendars-nav"></div><div class="calendar-dropdown"></div>{months}' +
			'{popup:start}<div class="calendars-ctrl"></div>{popup:end}' +
			'<div class="calendars-clear-fix"></div></div>',
			monthRow: '<div class="calendars-month-row">{months}</div>',
			month: '<div class="calendars-month"><div class="calendars-month-header">{monthHeader}</div><div class="hijre-calendar-header">{link:prev}<div class="calendar-text-content"><ul><li class="georgian-date">georgian-date</li><li class="Hijri-date">Hijri-date</li></ul></div>{link:next}</div>' +
		...
		commands: {
			...
			//alert($(picker.find(".calendar-text-content").html());
 	...
 	alert('Selected ' + dates);
		...
		_instSettings: function(elem, options) {
			if(typeof(options.iconUrl) != "undefined"){
				elem.after('<img class="ui-datepicker-trigger" id="'+elem.attr("id")+'_button" alt="picker" src="'+options.iconUrl+'"></img>');
			}else{
				elem.after('<button type="button" class="ui-datepicker-trigger" id="'+elem.attr("id")+'_button">...</img>');
			}
			if($('html').attr('lang')=="ar"){
				options.calendar=$.calendars.instance('gregorian', 'ar-EG');
				options.ar_en="ar";
			}
			try{
				$(formatString(("#"+elem.attr("id")+"_button"))).click(function(){
					elem.focus();
				});
			}catch(e){
				$((("#"+elem.attr("id")+"_button"))).click(function(){
					elem.focus();
				});
			}
 ...
 $(selector).datepick('setDate', [date1, date2, date3]) */
		...
		_generateContent: function(elem, inst) {//Test This Badawy
			var monthsToShow = inst.options.monthsToShow;
			//inst.options.calendar = $.calendars.instance("gregorian","ar-EG");
			...
			bigianDate = inst.options.calendar.parseDate("dd/mm/yyyy", "01/"+inst.drawDate.month()+"/"+inst.drawDate.year());
			monthDays = bigianDate.daysInMonth(inst.drawDate.year(), inst.drawDate.month());
			endDate = inst.options.calendar.parseDate("dd/mm/yyyy", monthDays + "/"+inst.drawDate.month()+"/"+ inst.drawDate.year());
			bigianHijreDate = $.calendars.instance('islamic',inst.options.ar_en).fromJD(bigianDate.toJD());
			endHijreDate = $.calendars.instance('islamic',inst.options.ar_en).fromJD(endDate.toJD());			
					
			formatedDate = inst.options.calendar.formatDate("MM yyyy",bigianDate);
			formatedBigianHijreDate	 = $.calendars.instance('islamic',inst.options.ar_en).formatDate("MM yyyy",bigianHijreDate);
			formatedEndHijreDate = $.calendars.instance('islamic',inst.options.ar_en).formatDate("MM yyyy",endHijreDate);
			
			picker.find(".georgian-date").text(formatedDate);//Morsi For georgian date @Badawy
			picker.find(".Hijri-date").text(formatedBigianHijreDate + " - "  + formatedEndHijreDate);//Morsi for Hijri-date @Badawy
		...
		_generateMonthSelection: function(inst, year, month, minDate, maxDate, monthHeader, calendar) {
			...
			var selector='<div class="month_label" for="Month_selector"><label>'+calendar.local["monthString"]+'</label>';
			 selector += '<select id="Month_selector" class="' + this._monthYearClass +
				' chosen-select" title="' + inst.options.monthStatus + '">';
			var maxMonth = calendar.monthsInYear(year) + calendar.minMonth;
			////////////////////////////////////////////////////////////My modification
			
			for (var m = calendar.minMonth; m < maxMonth; m++) {
			if(inst.options.showHijriData==false){
				if ((!minDate || calendar.newDate(year, m,
					...
					selector += '<option value="' + m + '/' + year + '"' +
						...
						monthNames[m - calendar.minMonth] + '</option>';// we can set month here @Badawy
				}
				}else{
					if ((!minDate || calendar.newDate(year, m,
						calendar.daysInMonth(year, m) - 1 + calendar.minDay).
						...
						bigianDate = inst.options.calendar.parseDate("dd/mm/yyyy", "01/"+m+"/"+year);
						monthDays = bigianDate.daysInMonth(year, m);
						endDate = inst.options.calendar.parseDate("dd/mm/yyyy", monthDays + "/"+m+"/"+ year);
						bigianHijreDate = $.calendars.instance('islamic',inst.options.ar_en).fromJD(bigianDate.toJD());
						endHijreDate = $.calendars.instance('islamic',inst.options.ar_en).fromJD(endDate.toJD());												

						bigianHijreMonth = $.calendars.instance('islamic',inst.options.ar_en).formatDate("MM",bigianHijreDate);
						endHijreMonth = $.calendars.instance('islamic',inst.options.ar_en).formatDate("MM",endHijreDate);
						
						selector += '<option value="' + m + '/' + year + '"' +
						(month === m ? ' selected="selected"' : '') + '>' +
						monthNames[m - calendar.minMonth] +" / "+bigianHijreMonth + " • "+ endHijreMonth+ ' </option>';
			...
			//var selector='<div class="month_label" for="Month_selector"><label>Month</label></div>';
			 //selector += '<div class="month_selector"><select id="Month_selector" class="' + this._monthYearClass +
			// Years
			selector ='<div class="year_label" for="year_selector"><label>'+calendar.local["yearString"]+'</label>';
			var yearRange = inst.options.yearRange;
			if(inst.options.showHijriData==false){
			if (yearRange === 'any') {
				selector += '<select class="chosen-select ' + this._monthYearClass + ' ' + this._anyYearClass +
			...
			if (yearRange === 'any') {
				...
				bigianDate = inst.options.calendar.parseDate("dd/mm/yyyy", "01/01/"+year);
				monthDays = bigianDate.daysInMonth(year, 12);
				endDate = inst.options.calendar.parseDate("dd/mm/yyyy", monthDays + "/12/"+ year);
				bigianHijreDate = $.calendars.instance('islamic',inst.options.ar_en).fromJD(bigianDate.toJD());
				endHijreDate = $.calendars.instance('islamic',inst.options.ar_en).fromJD(endDate.toJD());
					
				selector += '<select class="' + this._monthYearClass + ' ' + this._anyYearClass +
					'" title="' + inst.options.yearStatus + '">' +
					'<option>' + year + ' / '+bigianHijreDate.year() + ' • ' + endHijreDate.year() + '</option></select>' +
					'<input class="' + this._monthYearClass + ' ' + this._curMonthClass +
					month + '" value="' + year + '">';
			}
			else {
				...
				var addYear = function(y) {
					if (y !== 0 || calendar.hasYearZero) {
						...
						bigianDate = inst.options.calendar.parseDate("dd/mm/yyyy", "01/01/"+y);
						monthDays = bigianDate.daysInMonth(y, 12);
						endDate = inst.options.calendar.parseDate("dd/mm/yyyy", monthDays + "/12/"+ y);
						bigianHijreDate = $.calendars.instance('islamic',inst.options.ar_en).fromJD(bigianDate.toJD());
						endHijreDate = $.calendars.instance('islamic',inst.options.ar_en).fromJD(endDate.toJD());			
						...
						selector += '<option value="' +
							Math.min(month, calendar.monthsInYear(y) - 1 + calendar.minMonth) +
							'/' + y + '"' + (year === y ? ' selected="selected"' : '') + '>' +
							y +' / ' + bigianHijreDate.year() +' • '+ endHijreDate.year() + '</option>';
					}
				};
				if (start.toJD() < end.toJD()) {
					start = (minDate && minDate.compareTo(start) === +1 ? minDate : start).year();
	...
	$(function() {
		$(document).on('mousedown.' + pluginName, plugin._checkExternalClick).
			...
			function showDayOfYear(date) {        											
                return {content: date.day() + '<br><sub>' + $.calendars.instance('islamic').fromJD(date.toJD()).day() + '</sub>'};
            }
